...
int main(int argc, char const *argv[])
{
    ...
    vector<pair<int, pii> > b(q);
    for (pair<int, pii> &b_i : b)
        cin >> b_i.X >> b_i.Y.X >> b_i.Y.Y;
    ...
    for (pair<int, pii> &b_i : b) {
        if (b_i.X == 1) {
            sqd[(b_i.Y.X - 1) / m] += b_i.Y.Y - a[b_i.Y.X - 1];
            a[b_i.Y.X - 1] = b_i.Y.Y;
            continue;
        } else {
            ll ans = 0;
            ...
            int st = 0;
            while (st * m < b_i.Y.X - 1) st++;
            int ed = 0;
            while ((ed + 1) * m < b_i.Y.Y - 1) ed++;
            
            for (int i = b_i.Y.X - 1; i < min(b_i.Y.Y, st * m); i++)
                ans += a[i];
            for (int i = st; i < ed; i++)
                ans += sqd[i];
            for (int i = max(st, ed) * m; i < b_i.Y.Y; i++)
                ans += a[i];
